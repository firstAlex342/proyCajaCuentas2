...
{
    public partial class FrmBuscarFoliosActivosDeTodosImprimirExportar : Form
    {
        //-------------------constructor
        public FrmBuscarFoliosActivosDeTodosImprimirExportar()
        {
            InitializeComponent();
        }

        //----------------Methods controller
        private async Task<DataTable> Socio_BuscarFoliosActivosDeTodosEnReciboListaProductosControllerAsync(DateTime fechaInicio, DateTime fechaFin)
        {
            ClsSocio clsSocio = new ClsSocio();
            clsSocio.FechaAlta = fechaInicio;
            clsSocio.FechaModificacion = fechaFin;

            DataTable respuesta = clsSocio.Socio_BuscarFoliosActivosDeTodosEnReciboListaProductos();
            await Task.Delay(10);
            return (respuesta);
        }
